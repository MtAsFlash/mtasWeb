...
//	    skin : 'row',
            ...
            request: {
                ...
                serverId: $.result(fsConfig, "global.page.request.serverId", "serverId") //页码的参数名称，默认：pageNum
            },
            response: {
                ...
                if (url === "/gamelog/getAdjacentLogs.json") {
                    currPage = curr;
                ...
                // 工具栏按钮权限过滤
                if ($("#buttonPermissions").val() != undefined) {
                    var permissions = $("#buttonPermissions").val().trim().replace("[", "").replace("]", "").split(",");
                    $("a[permission]").hide();
                    $.each(permissions, function (index, value) {
                        $("a[permission='" + value.trim() + "']").show();
                    });
                }
            }
        });
            ...
            switch (layEvent) {
                ...
                function submitForm() {
                    ...
                    if ($.isEmpty(funcNo) && $.isEmpty(url)) {
                        fsCommon.warnMsg("fsData功能号或请求地址为空！");
                        return;
                    }
